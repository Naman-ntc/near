...
class IntersectionScenario(gym.Env):
    def __init__(self, visualize=True, random_perturbations = False, **kwargs):
    ...
    def add_noise(self):
        self.ego.center += Point(0, 20*self.np_random.random() - 10)
        self.adv.center += Point(20*self.np_random.random() - 10, 0)
        self.ego_reaction_time += self.np_random.random() - 0.5
    ...
    def get_cautious_ego_control(self, ttc_ego, ttc_adv):
        ...
        if ttc_ego < 0.05 or ttc_adv < 0:
            return np.array([0, 1.95 + 0.05 * self.np_random.random() * self.random_perturbations], dtype=np.float32)
        elif ttw_ego > 1.0 and ttw_ego < 4.5:
            return np.array([0, 0], dtype=np.float32)
        elif ttc_ego < ttc_adv - 0.3 or not self.ego_can_see_adv:
            return np.array(
                [0, np.minimum(1.0, np.maximum(0.4, self.ego.inputAcceleration + 0.2 * self.np_random.random() * self.random_perturbations - 0.1))],
                dtype=np.float32)
        else:
    ...
    def get_left_turn_ego_control(self, ttc_ego, ttc_adv):
        ...
        if ttc_ego < -1 or ttc_adv < -1:
            if ttw_left_ego < -1.0:
                return np.array([0.75, 1.25 + 0.05 * self.np_random.random() * self.random_perturbations], dtype=np.float32)
            else:
            ...
            return np.array([0, -2.75 - np.random.rand() * 0.25 * self.random_perturbations], dtype=np.float32)
    ...
    def get_aggressive_ego_control(self, ttc_ego, ttc_adv):
        if ttc_ego < 0.05 or ttc_adv < 0:
            ...
            return np.array(
                [0, np.minimum(2.0, np.maximum(1.2, self.ego.inputAcceleration + 0.2 * self.np_random.random() * self.random_perturbations - 0.1))],
                dtype=np.float32)
        else:
    ...
    def get_right_turn_ego_control(self):
        ...
        if ttc_ego < ttc_adv - 0.6 or not self.ego_can_see_adv:
            # slowly accelerate?
            return np.array(
                [0, np.minimum(0.7, np.maximum(0.4, self.ego.inputAcceleration + 0.2 * self.np_random.random() * self.random_perturbations - 0.1))],
                dtype=np.float32)
        if tty_ego > 0:
        ...
        if ttw_ego > -7.60:
            ...
            return np.array([0.415, 0.55 + 0.05 * self.np_random.random() * self.random_perturbations], dtype=np.float32)  # accelerate out
        else:
            return np.array([0, 0.2], dtype=np.float32)
    ...
    def get_ego_control(self,policy_no=1):
        if policy_no == 0:  # aggressive
            ...
            return self.get_cautious_ego_control(*self.compute_ttcs())
    ...
    def compute_ttcs(self):
        ...
        return self.ttc_ego, self.ttc_adv
...
if __name__ == '__main__':
    ...
    def buildOpts():
        ...
        parser.add_argument('--num_episodes_train', type=int, default=500)
        parser.add_argument('--num_episodes_test', type=int, default=200)
        parser.add_argument('--max_steps', type=int, default=150)
        parser.add_argument('--seed', type=int, default=0)
        parser.add_argument('--random_perturbations', type=int, default=0) ## boolean
        parser.add_argument('--outpathprefix', default='out') ## boolean
        parser.add_argument('--data_collection', type=int, default=1)
        return parser.parse_args()

    args = buildOpts()

    states_for_imitation = []
    actions_for_imitation = []
    for _ in tqdm(range(args.num_episodes_train)):
        env = IntersectionScenario(not args.data_collection, args.random_perturbations)
        for _ in range(args.max_steps):
            info = env.step(None)[-1]
            state, act = info['previous_state_action']
            if not args.data_collection:
                env.render()
            else:
                states_for_imitation.append(state)
                actions_for_imitation.append(act)
            time.sleep(0.1/4)
        env.close()
    np.save(args.outpathprefix + 'carlotrain_X.npy', np.array(states_for_imitation))
    np.save(args.outpathprefix + 'carlotrain_Y.npy', np.array(actions_for_imitation))
    
    states_for_imitation = []
    actions_for_imitation = []
    for _ in tqdm(range(args.num_episodes_test)):
        env = IntersectionScenario(not args.data_collection, args.random_perturbations)
        for _ in range(args.max_steps):
            info = env.step(None)[-1]
            state, act = info['previous_state_action']
            ...
            time.sleep(0.1/4)
        env.close()
    np.save(args.outpathprefix + 'carlotest_X.npy', np.array(states_for_imitation))
    np.save(args.outpathprefix + 'carlotest_Y.npy', np.array(actions_for_imitation))
